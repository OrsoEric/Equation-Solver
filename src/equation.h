...
class Equation
{
	...
	*********************************************************************************************************************************************************/
	...
	struct Token
	{
		...
		{
			return Equation::to_string( (*this) );
		}
	};
	...
	*********************************************************************************************************************************************************/
	...
	//parse are the methods that translate a string into a tree of tokens. There are overloads depending on what the user needs to do and if user needs intermediate rapresentations
	//fully expose intermediate results of the parsing. the array of string tokens, and the array of tokens
	bool parse( std::string is_equation, std::vector<std::string>& oras_string_tokens, std::vector<Equation::Token> &orast_tokens )
	{
		return Equation::parse( is_equation, oras_string_tokens, orast_tokens, this->gcl_token_tree );
	}
	//Parse a string as an equation, the user doesn't care about intermediate results
	bool parse( std::string is_equation )
	{
		std::vector<std::string> as_string_tokens;
		std::vector<Equation::Token> ast_tokens;
		return Equation::parse( is_equation, as_string_tokens, ast_tokens, this->gcl_token_tree );
	}
	//Parse a string as an equation, the user doesn't care about intermediate results
	bool parse( const char *ipcs8_equation_string )
	{
		std::vector<std::string> as_string_tokens;
		std::vector<Equation::Token> ast_tokens;
		return Equation::parse( std::string( ipcs8_equation_string ), as_string_tokens, ast_tokens, this->gcl_token_tree );
	}
	...
	*********************************************************************************************************************************************************/
	...
	static std::string to_string( Token &irst_token );
	//reverse translate a tree into a token array into a string and return the string
	bool to_string( std::string &ors_equation );
	std::string to_string( void );
	...
	*********************************************************************************************************************************************************/
	...
	//! @brief true = token is an operator
	static bool is_token( Token &irst_source, Token_type ie_token_type )
	{
		return (irst_source.e_type == ie_token_type);
	}
	...
	*********************************************************************************************************************************************************/
	...
	static bool parse( std::string is_equation, std::vector<std::string>& oras_string_tokens, std::vector<Equation::Token> &orast_tokens, User::Tree<Equation::Token> &oracl_tree_tokens );
	//Equation Tokenizer. Translates an equation in string form to an array of string tokens.
	static bool convert_equation_string_to_array_token( std::string is_equation, std::vector<std::string> &oras_token_array, std::vector<Token> &orast_token_array );
	...
	//Rverse translation from an array of tokens to a string
	static bool convert_token_array_to_string( std::vector<Token> &irast_token_array, std::string &ors_equation );
